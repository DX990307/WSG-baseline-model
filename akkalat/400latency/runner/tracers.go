...
// type GMMUTLBTracer struct {
// 	tracer *GMMUCacheEvictTracer
// 	tlb    *tlb_gmmu.GMMUTLB
// }
...
func (r *Runner) addGMMUCacheHitRateTracer() {
	...
	for _, gpu := range r.platform.GPUs {
		tracer := tracing.NewStepCountTracer(
			func(task tracing.Task) bool { return true })
		r.gmmuCacheHitRateTracers = append(
			r.gmmuCacheHitRateTracers,
			gmmuCacheHitRateTracer{
				tracer:    tracer,
				gmmuCache: gpu.GMMUTLB,
			},
		)
		tracing.CollectTrace(gpu.GMMUTLB, tracer)
	}
}
...
func (r *Runner) addGMMUCacheLatencyTracer() {
	...
	for _, gpu := range r.platform.GPUs {
		...
		r.gmmuCacheLatencyTracers = append(
			r.gmmuCacheLatencyTracers,
			gmmuCacheLatencyTracer{
				tracer:    tracer,
				gmmuCache: gpu.GMMUTLB,
			},
		)
...
// func (r *Runner) addGMMUTLBTracer() {
// 	for _, gpu := range r.platform.GPUs {
// 		t := GMMUTLBTracer{}
// 		t.tlb = gpu.GMMUTLB
...
// 		r.GMMUTLBTracers = append(r.GMMUTLBTracers, t)
